...
#---- Initialization of Samples per Channel variable for this code only (not imported from config.py here)
nSpC = 21
SpC = np.zeros(nSpC)
SpC[0] = float(1e0)
SpC[1] = float(2e0)
...
DL = pd.DataFrame(np.zeros((nDev,nSpC),dtype='float'))
...
PBElapsed = np.zeros(nSpC) # For use in determining time taken to obtain measurements from USB6216
PBAverage = np.zeros(nSpC) # For use in determining time taken to obtain measurements from USB6216
GrabStart = np.zeros(nSpC) # For use in determining time taken to run a grab
GrabEnd = np.zeros(nSpC) # For use in determining time taken to run a grab
GrabTime = np.zeros(nSpC) # for use in determining time taken to run a grab
GrabTime[:] = np.nan
...
with open(dataPath + '/log_'+t+'_'+measurementName+'.txt', 'w') as fLog:
    fLog.write('Start: '+str(datetime.now()) + '\n' +
               ...
               'NIDAQ Samples per Channel: variable' + '\n' +
               'Number of Grabs: ' + str(nSpC) + '\n'
               )
...
def updateGUI(): # Updates the data in the GUI -- last edited APM 19Jan24
    ...
    grabTot = tk.Label(root, text=('of total grabs: '+ str(nSpC)),bg="skyblue")
...
#        print(f'DR = {DR.iloc[i,nGrab]:.2f} +/- {DRerr.iloc[i,nGrab]:.2f} uS') ## Keep for diagnostics; Off from 15JAN24 APM
        ...
        RD[(2*nDevL+1)] = round(DLerr.iloc[i,nGrab],3)
        RD[(2*nDevR)] = round(DR.iloc[i,nGrab],3)
        RD[(2*nDevR+1)] = round(DRerr.iloc[i,nGrab],3)
        #---- send data to file
        with open(runPath+'/'+t+'_'+measurementName+'_G'+str(nRun)+'_Dev'+str(nDevL)+'.csv', 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([str(nGrab+1),str(samp),str(DL.iloc[i,nGrab]),str(DLerr.iloc[i,nGrab]),str(datetime.now().strftime("%H:%M:%S"))])
    ...
    with open(runPath+'/'+t+'_'+measurementName+'_G'+str(nRun)+'.csv','a',newline='') as f:
        writer = csv.writer(f)
        writer.writerow(RD[:])
#    ResData.to_csv(runPath+'/'+t+'_'+measurementName+'_G'+str(nRun)+'.csv', index=False)
...
def measLoop():
    ...
    for i in range(nSpC): #Edited from ItersAR for this code APM 27MAR24
        ...
        with open('stop.txt', 'r') as fStop:
            ...
            if r == 'stop':
                ...
                break
        #GT = WaitAR-GrabTime[i] -- Turned off for this code APM 27MAR24
    ...
    with open(dataPath + '/log_'+t+'_'+measurementName+'.txt', 'a') as fLog:
        fLog.write('Measurement '+measurementName+'G'+str(nRun)+' finished at: '+str(datetime.now())+'\n'+
                   'with '+str(nGrab+1)+' of '+str(nSpC)+' grabs completed.'+'\n \n'
                   )
    nRun += 1
