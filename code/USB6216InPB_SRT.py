...
APM 27MAR24 -- This is a special version that enables the samples per channel (SpC) to be pulled as a variable from SampleRateTest
...
class USB6216InPB_SRT(Instrument.Instrument):
    ...
    def _getInputLevel(self):
        with nmx.Task() as task:
            ...
            task.timing.cfg_samp_clk_timing(rate=SR, sample_mode=constants.AcquisitionType.CONTINUOUS, samps_per_chan=self.SpC)
            reader = stream_readers.AnalogMultiChannelReader(task.in_stream)
            buffer = np.zeros((2, self.SpC), dtype = np.float64)
